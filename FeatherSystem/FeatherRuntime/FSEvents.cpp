...
bool FSEvents::enqueue_dispatch_for_index(size_t index, uint64_t now_ms){ if(!scheduler_||index>=slots_.size()) return false; auto& slot=slots_[index]; if(!slot.occupied||!slot.event.condition_matches(now_ms)||!slot.event.mark_dispatch_pending()) return false; uint32_t gen=slot.generation, epoch=slot.event.dispatch_epoch; uint8_t pri=slot.event.priority; if(!scheduler_->enqueue_ready_task([this,index,gen,epoch,now_ms](){ if(index>=slots_.size()) return; auto& s=slots_[index]; if(!s.occupied||s.generation!=gen) return; auto& e=s.event; if(!e.consume_pending_dispatch(epoch)||!e.enabled) return; e.dispatch(now_ms); if(s.pending_destroy && !e.dispatching){ s.event=EventRecord{}; s.pending_destroy=false; s.enabled_position=InvalidPosition; free_indices_.push_back(index);} }, pri)){ slot.event.cancel_pending_dispatch(); return false; } return true; }
